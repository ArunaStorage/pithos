...
impl TryFrom<&[u8]> for Crypt4GHHeader {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        ...
        header.packet_count = cursor.read_u32::<LittleEndian>().map_err(|_| Crypt4GHError::FromBytesError("header size".to_string()))?;
        while cursor.position() < header.packet_count as u64 {
            let len = cursor.read_u32::<LittleEndian>().map_err(|_| Crypt4GHError::FromBytesError("packet length".to_string()))?;
            let mut buf = vec![0; len as usize];
            cursor.read_exact(&mut buf).map_err(|_| Crypt4GHError::FromBytesError("packet data".to_string()))?;
            header.header_packets.push(HeaderPacket::from_buf(&mut buf, len as usize)?);
        }
        Ok(header)
    }
}
...
impl HeaderPacket {
    ...
    pub fn from_buf(bytes: &mut [u8], len: usize) -> Result<Self, Crypt4GHError> {
        let encryption_method =  bytes.read_u32::<LittleEndian>().map_err(|_| Crypt4GHError::FromBytesError("encryption method".to_string()))?;
        let mut writers_pubkey = [0; 32];
        bytes.read_exact(&mut writers_pubkey).map_err(|_| Crypt4GHError::FromBytesError("writer's public key".to_string()))?;
        let mut nonce =  [0; 12];
        bytes.read_exact(&mut nonce).map_err(|_| Crypt4GHError::FromBytesError("nonce".to_string()))?;
        let encrypted_packet_data = PacketData::Encrypted(bytes[4+32+12..bytes.len() - 16].to_vec());
        ...
        Ok(HeaderPacket {
            ...
            mac: mac.try_into().map_err(|_| Crypt4GHError::FromBytesError("packet mac".to_string()))?,
        })
    }
    ...
    pub fn decrypt(&mut self, readers_private_key: SecretKey) -> Result<(), Crypt4GHError> {
        ...
        let session_key = Keypair::from(readers_private_key).session_keys_from(&writers_pub_key).rx.as_ref();
        self.packet_data.decrypt(session_key.into(), &self.nonce, &self.mac)?;
        Ok(())
    }

    pub fn encrypt(&mut self, readers_pubkey: PublicKey, writers_private_key: Option<SecretKey>) -> Result<(), Crypt4GHError> {
...
impl PacketData {
    pub fn decrypt(&mut self, session_key: &[u8; 32], nonce: &[u8; 12], mac: &[u8; 16]) -> Result<(), Crypt4GHError> {
        if let Self::Encrypted(enc_data) = &self {
            let decrypted_bytes = ChaCha20Poly1305::new_from_slice(session_key)
                ...
                .map_err(|e| {
                    Crypt4GHError::DecryptionFailed
                })?;

            self = &mut Self::Decrypted(Self::packet_from_bytes(&decrypted_bytes)?);
        } else {
            return Err(Crypt4GHError::DecryptionError("packet data is not encrypted".to_string()));
        }
        Ok(())
    }

    pub fn encrypt(&mut self, session_key: &[u8; 32], nonce: &[u8; 12]) -> Result<[u8; 16], Crypt4GHError> {
        return if let Self::Decrypted(dec_data) = &self {
            ...
            ChaCha20Poly1305::new_from_slice(session_key)
                .map_err(|e| {
                    Crypt4GHError::EncryptionError(format!("initialize encryptor: {}", e))
                })?
                .encrypt(nonce.into(), enc_data.as_slice())
                .map_err(|e| {
                    Crypt4GHError::EncryptionError("encrypt chunk".to_string())
                })?;
            self = &mut Self::Encrypted(enc_data[..enc_data.len() - 16].to_vec());
            Ok(enc_data[enc_data.len() - 16..].try_into().map_err(|_| Crypt4GHError::EncryptionError("packet mac".to_string()))?)
        } else {
            Err(Crypt4GHError::EncryptionError("packet data is already encrypted".to_string()))
        }
    }
    ...
    pub fn packet_from_bytes(bytes: &[u8]) -> Result<Vec<Packet>, Crypt4GHError> {
        ...
        while cursor.position() < bytes.len() as u64 {
            let packet_type = cursor.read_u32::<LittleEndian>().map_err(|_| Crypt4GHError::FromBytesError("packet type".to_string()))?;

            match packet_type {
                0 => {
                    let encryption_method = cursor.read_u32::<LittleEndian>().map_err(|_| Crypt4GHError::FromBytesError("encryption method".to_string()))?;
                    if encryption_method != 0 {
                        return Err(Crypt4GHError::InvalidSpec("unsupported encryption method".to_string()));
                    }
                    let mut encryption_key = [0; 32];
                    cursor.read_exact(&mut encryption_key).map_err(|_| Crypt4GHError::FromBytesError("encryption key".to_string()))?;
                ...
                1 => {
                    if found_edit {
                        return Err(Crypt4GHError::InvalidSpec("multiple edit lists not allowed".to_string()));
                    }
                    let num_length = cursor.read_u32::<LittleEndian>().map_err(|_| Crypt4GHError::FromBytesError("number of edits".to_string()))?;
                    let mut edits = Vec::new();
                    for _ in 0..num_length {
                        edits.push(cursor.read_u64::<LittleEndian>().map_err(|_| Crypt4GHError::FromBytesError("edit".to_string()))?);
                    }
                ...
                _ => {
                    return Err(Crypt4GHError::FromBytesError("invalid packet type".to_string()));
                }
            }
        }
        Ok(packets)
